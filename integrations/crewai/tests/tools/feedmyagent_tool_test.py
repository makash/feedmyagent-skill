...
level, the same way upstream tests mock `requests.post` for HTTP-backed
tools (see tests/tools/serper_dev_tool_test.py in crewAI-tools).
"""
...
@patch(PATCH_TARGET)
def test_latest_tool_formats_items(mock_fma_cls):
    ...
    mock_fma_cls.assert_called_once_with(base_url="https://api.feedmyagent.com", user_agent="feedmyagent-crewai/0.1")
...
@patch(PATCH_TARGET)
def test_latest_tool_no_results(mock_fma_cls):
    mock_client = MagicMock()
...
@patch(PATCH_TARGET)
def test_latest_tool_formats_api_error(mock_fma_cls):
    mock_client = MagicMock()
...
@patch(PATCH_TARGET)
def test_search_tool_passes_query_through(mock_fma_cls):
    mock_client = MagicMock()
...
def test_search_tool_args_schema_requires_text():
    from crewai_tools.tools.feedmyagent_tool.feedmyagent_tool import FeedMyAgentSearchInput

    with pytest.raises(Exception):
        FeedMyAgentSearchInput()  # missing required `text`
...
@patch(PATCH_TARGET)
def test_report_tool_posts_and_formats_confirmation(mock_fma_cls):
    mock_client = MagicMock()
    mock_client.report.return_value = _item(id="pending-1", title="Reported title")
    mock_fma_cls.return_value = mock_client

    tool = FeedMyAgentReportTool(api_key="ask_test123")
    result = tool.run(title="Reported title", description="Full details...", url="https://x.example/1")

    mock_fma_cls.assert_called_once_with(
        api_key="ask_test123", base_url="https://api.feedmyagent.com", user_agent="feedmyagent-crewai/0.1"
    )
...
@patch(PATCH_TARGET)
def test_report_tool_surfaces_missing_api_key_error(mock_fma_cls):
    mock_client = MagicMock()
